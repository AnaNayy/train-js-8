// Завдання 1
// В даному завданні створимо функцію, яка використовує метод `Array.from` для
// створення масиву зі списку користувачів з бази даних.
...
function getUsersFromDatabase(array) {
  // Використовуємо метод `Array.from` для створення масиву користувачів зі списку, елементи
  //якого це об'єкти які міститять id який дорівнює id користувача,firstName який дорівнює
  //firstName користувача в верхньому регістрі та years який дорівнює age користувача
...
/**
 * Функція `removeLastElement` видаляє останній елемент з масиву, використовуючи метод
 * `pop`.
...
/**
 * Функція `checkArray` перевіряє, чи є вхідний об'єкт масивом, використовуючи метод
 * `Array.isArray`.
...
function checkArray(obj) {
  ...
  if (obj === null || obj === undefined) {
    console.error("Вхідний об'єкт є null або undefined.");
    return false;
  }
...
/**
 * Функція `createArray` створює новий масив з переданих елементів, використовуючи метод
 * `Array.of`.
...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.error(
      "Помилка: Не передано жодного елементу для створення масиву."
    );
    return [];
  }
}
...
/**
 * Функція `getElementAtIndex` отримує елемент з масиву за заданим індексом, використовуючи
 *  метод `at`.
 *  arr - Вхідний масив.
...
/**
 * Копіює та переставляє задану кількість елементів з масиву в нове місце за допомогою
 *  методу `copyWithin`.
...
/**
 * Функція `sortByKey` сортує вхідний масив об'єктів за допомогою методу `sort`
 * та заданого ключа.
...
function sortByKey(arr, key) {
  // Використовуємо метод `sort` передаємо в нього два аргументи a та b, для сортування
  // масиву об'єктів за заданим ключем
...
/**
 * Функція `customEvery` перевіряє, чи всі елементи масиву задовольняють заданому умову за
 * допомогою методу `every`.
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  // Використання методу `fill` для заповнення вказаного діапазону елементів масиву заданим
  // значенням
...
function customIncludes(arr, element) {
  ...
  // За допомогою методу filter перевіряємо скільки разів в масиві зустрічається
  // елемент та виводимо число в консоль
  ...
  const filtered = arr.filter((item) => item === element).length;
